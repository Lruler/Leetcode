...
function DoublyLinkedList() {
  ...
  DoublyLinkedList.prototype.append = function (data) {
    // 1.根据data创建节点
    var newNode = new Node(data); // 2.判断添加的是否为第一个节点
  ...
  DoublyLinkedList.prototype.insert = function (position, data) {
    ...
    var newNode = new Node(data); // 3.判断此时链表是否为空
  ...
  DoublyLinkedList.prototype.removeAt = function (position) {
    ...
    if (this.length == 1) {
      ...
      if (position == 0) {
        ...
        this.head.prev = null; //3.3、position=this.length-1
      } else if (position == this.length - 1) {
        console.log('执行');
        this.tail = this.tail.prev;
        this.tail.next = null; //3.2、position=middle
      } else {
        var current = this.head;
        var index = 0;
...
rl.question('请输入链表长度:', function (a) {
  rl.question('请输入链表中的元素:', function (str) {
    rl.question('请输入你要逆置的数量:', function (n) {
      var len = str.length;
      ...
      for (var _i = +n - 1; _i >= 0; _i++) {
        if (_i == +n - 1) {
          var _node = list1.get(+n);

          var node1 = list1.get(_i);
          _node.prev = list1.head;
          list1.head = node1;
          node1.prev = null;
          node1.next = _node.prev;
        } else {
